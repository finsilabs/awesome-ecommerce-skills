...
export function generateReadme(catalog, evalResults) {
  ...
  lines.push('- [Works With Your Platform](#works-with-your-platform)');
  ...
  lines.push('## Works With Your Platform');
  lines.push('');
  lines.push('Skills work with **any** e-commerce platform. They teach your AI assistant how to write code that connects to your specific stack — whether that\'s Shopify, WooCommerce, Magento, or a custom headless setup.');
  lines.push('');
  lines.push('### Shopify');
  lines.push('');
  lines.push('Skills guide your AI to use Shopify\'s actual APIs — Admin API, Storefront API, and Checkout Extensions:');
  lines.push('');
  lines.push('```');
  lines.push('You: "Add a loyalty points system to my Shopify store"');
  lines.push('');
  lines.push('AI + loyalty-points-system skill:');
  lines.push('  → Uses Shopify Admin API to store points via metafields');
  lines.push('  → Creates a Shopify Function for automatic discount application');
  lines.push('  → Adds a points display widget via App Blocks');
  lines.push('  → Listens to orders/create webhook to award points');
  lines.push('```');
  lines.push('');
  lines.push('We also have 7 **Shopify-specific skills** covering the Admin API, Storefront API, theme development, checkout extensions, webhooks, metafields, and app development.');
  lines.push('');
  lines.push('### WooCommerce');
  lines.push('');
  lines.push('Skills produce code that hooks into WooCommerce\'s plugin architecture:');
  lines.push('');
  lines.push('```');
  lines.push('You: "Build a subscription box product for my WooCommerce store"');
  lines.push('');
  lines.push('AI + woocommerce-subscriptions skill:');
  lines.push('  → Creates a WooCommerce plugin with proper hooks (woocommerce_subscription_status_updated)');
  lines.push('  → Integrates with WooCommerce Subscriptions API for recurring billing');
  lines.push('  → Adds subscription management to My Account page');
  lines.push('  → Handles renewal orders and failed payment retry');
  lines.push('```');
  lines.push('');
  lines.push('We have 5 **WooCommerce-specific skills** for plugin development, Blocks, REST API, subscriptions, and performance.');
  lines.push('');
  lines.push('### Headless / Custom Platforms');
  lines.push('');
  lines.push('For headless setups (Next.js, Remix, etc.), skills provide complete backend + frontend code:');
  lines.push('');
  lines.push('```');
  lines.push('You: "Add real-time inventory tracking to my headless store"');
  lines.push('');
  lines.push('AI + inventory-tracking skill:');
  lines.push('  → Creates PostgreSQL schema with version-based optimistic locking');
  lines.push('  → Builds REST/GraphQL API for stock checks and reservations');
  lines.push('  → Adds cart reservation with automatic TTL expiry');
  lines.push('  → Implements multi-warehouse availability aggregation');
  lines.push('```');
  lines.push('');
  lines.push('We have 8 **headless-specific skills** covering Shopify Hydrogen, Medusa, Saleor, Commerce.js, JAMstack, and PWA storefronts.');
  lines.push('');
  lines.push('### Also Supported: Magento (4 skills) and Salesforce Commerce Cloud (3 skills)');
  lines.push('');
  lines.push('> **How does this actually work?** You copy a skill file into your project (or install via `tessl install`). Your AI assistant reads it automatically — no configuration needed. The next time you ask it to build something related, it follows the skill\'s instructions instead of guessing. The skill tells it which APIs to call, what patterns to use, and what mistakes to avoid for your specific platform.');
  lines.push('');
